...
impl<'txn> Interpreter<'txn> {
        ...
        loop {
            ...
            match exit_code {
                ...
                ExitCode::Call(fh_idx) => {
                    ...
                    if func.is_native() {
                        let caller = current_frame.function.module_id();
                        self.call_native(&resolver, context, caller, func, vec![])?;
                        continue;
                    }
                ...
                ExitCode::CallGeneric(idx) => {
                    ...
                    if func.is_native() {
                        let caller = current_frame.function.module_id();
                        self.call_native(&resolver, context, caller, func, ty_args)?;
                        continue;
                    }
                    // TODO: when a native function is executed, the current frame has not yet
                    // been pushed onto the call stack. Fix it.
    ...
    fn call_native(
        ...
        let mut native_context =
            FunctionContext::new(caller_module, module_id, self, context, resolver);
