            ...
            match exit_code {
                ...
                ExitCode::Call(fh_idx) => {
                    ...
                    cost_strategy
                        ...
                        .map_err(|e| set_err_info!(current_frame, e))?;
                    if func.is_native() {
                        self.call_native(&resolver, data_store, cost_strategy, func, vec![], current_frame.function.module_id())?;
                        current_frame.pc += 1; // advance past the Call instruction in the caller
                        continue;
                    }
                    let frame = self
                ...
                ExitCode::CallGeneric(idx) => {
                    ...
                    cost_strategy
                        ...
                        .map_err(|e| set_err_info!(current_frame, e))?;
                    if func.is_native() {
                        self.call_native(&resolver, data_store, cost_strategy, func, ty_args, current_frame.function.module_id())?;
                        current_frame.pc += 1; // advance past the Call instruction in the caller
                        continue;
                    }
                    let frame = self
